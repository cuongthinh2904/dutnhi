...
module.exports.handleReply = async function ({ event, Users, api, handleReply, Currencies }) {
  if (handleReply.type == "answer") {
    ...
    if (senderID !== handleReply.author) return api.sendMessage("Bạn không phải người chơi⚠️", threadID, messageID);
    ...
    if (choose == "HELP 1" || choose == "HELP1") {
      if (senderInfo.data.helpaltp.helpm !== 1) return api.sendMessage("Bạn đã dùng quyền trợ giúp này rồi⚠️", threadID, messageID);
      ...
      var fuckk = `Hệ thống đã loại bỏ phương án sai là ${mot[0]} và ${mot[1]}`;
    ...
    if (senderInfo.data.helpaltp.helpm == 2 && (choose == mot[0] || choose == mot[1])) return api.sendMessage("Đáp án này đã bị loại bỏ⚠️", threadID, messageID);

    if (choose == "HELP 2" || choose == "HELP2") {
      if (senderInfo.data.helpaltp.helph !== 1) return api.sendMessage("Bạn đã dùng quyền trợ giúp này rồi⚠️", threadID, messageID);
  var linkhai = hai.length == 1 ? hai[0] : senderInfo.data.helpaltp.helpm == 2 ? hai[1] : hai[0];
      ...
      await Users.setData(senderID, senderInfo);
      return api.sendMessage({
        body: "Đây là kết quả khảo sát của khán giả!",
        attachment: fs.createReadStream(pathhelp)
      }, threadID, () => fs.unlinkSync(pathhelp), messageID);
    }

    if (choose == "HELP 3" || choose == "HELP3") {
      if (senderInfo.data.helpaltp.helpb !== 1) return api.sendMessage("Bạn đã dùng quyền trợ giúp này rồi⚠️", threadID, messageID);
      api.unsendMessage(handleReply.messageID);
      let background = await loadImage(loz);
      let tuvan1 = await loadImage(getlink(3, ba[0]));
      let tuvan2 = await loadImage(getlink(3, ba[1]));
    ...
    if (choose == handleReply.dapandung) {
      ...
      if (levelcc < 15) {
        ...
        api.sendMessage(`➝ ${choose} là đáp án chính xác, ${handleReply.giaithich}\n\n➝ Xin chúc mừng người chơi ${name} đã xuất sắc trả lời đúng ${djtme}\nnâng mức phần thưởng lên ${equi(levelcc)} đô`, threadID, messageID);
...
+ prefix + "ALTP REGISTER ➝ Đăng kí chương trình (cần 1000 đô)" + "\n"
...
+ prefix + "ALTP STOP ➝ Dừng chơi và nhận tiền thưởng"
  ...
  if (type == "register") {
    ...
    if (money < moneydown) return api.sendMessage(`➝ Bạn không có đủ ${moneydown} đô để đăng kí, vui lòng theo thầy Huấn làm ăn bươn chải!`, threadID, messageID);
    return api.sendMessage(`➝ Thả icon vào tin nhắn này để xác nhận dùng ${moneydown} đô đăng kí tham gia chương trình!`, threadID, (error, info) => {
  ...
  if (type == "stop") {
    ...
    return api.sendMessage(`➝ Thả icon vào tin nhắn này để xác nhận dừng cuộc chơi tại đây và ra về với phần thưởng ${equi(abc)} đô`, threadID, (error, info) => {
      global.client.handleReaction.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
  ...
  if (type == "play") {
    try {
      ...
      if (level == 15) {
        ...
        senderInfo.data.altp = { level: -1, rd: -1 };
        await Users.setData(senderID, senderInfo);
        return api.sendMessage({ body: `➝ Xin chúc mừng người chơi ${name} đã xuất sắc vượt qua 15 câu hỏi của chương trình mang về 250000 đô\nHẹn gặp lại bạn ở chương trình lần sau!`, attachment: await makeWinner(senderID, 15)}, threadID, () => fs.unlinkSync(path), messageID);
      }
      var cauhoi = level + 1;
      const res = await axios.get(`https://raw.githubusercontent.com/KhoaDo472005/ailatrieuphu/main/altp${cauhoi}.json`);
