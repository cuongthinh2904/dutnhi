...
promise.then(async(r) => api.sendMessage(`✅ Gửi thông báo thành công đến tất cả nhóm`, tid, mid)).catch(async(err) => api.sendMessage(`❌ Không thể gửi thông báo đến ${err} nhóm`, tid, mid))
};
module.exports.handleReply = async({ api, event, handleReply: h, Users, Threads }) => {
    ...
    switch (h.type) {
     ...
     if (e) r2(++c2); else r1(++c1)
          ...
          promise.then(async(r1) => api.sendMessage(`📨 Phản hồi thành công đến ${(await Users.getData(h.author)).name} và ${+r1-1} Admin khác`, tid, mid)).catch(async(err) => api.sendMessage(`❌ Không thể phản hồi đến ${err} Admin khác`, tid, mid))
            break;
        };
    case "adminReply": {
        const atm = atms.length != 0 ? atms : "nofile";
